...
constructor(util)
{
	...
	this.SampleRate = 50;
	this.TraversalRate = 200;
	this.Distance_differential = 0;
	this.GPSInterval = 0;
	this.start_auto = false;
	this.propulsion_speed = 50;
	...
	this.GPSInterval = setInterval(() =>
	{
		...
		this.model.set("NeoCortex",
		{
			...
			Automation: this.start_auto,
			Propulsion_Speed: this.propulsion_speed
		});
	}, this.SampleRate);\

	this.TaversalInterval = setInterval(() =>
	{
		if(this.start_auto)
		{
			this.Distance_differential = this.distanceGPS(
				this.GPS_current.lattitude,
				this.GPS_current.longitude,
				this.GPS_gate.lattitude,
				this.GPS_gate.longitude
			);

			this.log.debug1(this.Distance_differential);

			if (this.Distance_differential < 1.5)
			{
				this.clearAutoGPS();
			}
			else
	...
	this.Previous_Distance = Infinity;
	this.Stopped_Counter = 0;
	// this.STOPPED_INCREMENTER_LIMIT = 10;
	this.STOPPED_INCREMENTER_LIMIT = Infinity;
	this.DriveRate = 1000;

	this.DriveInterval = setInterval(() =>
	{
		if(this.Distance_differential-0.5 < this.Previous_Distance && this.Previous_Distance < this.Distance_differential+0.5)
		{
			this.Stopped_Counter++;
		}
		else
		{
			this.propulsion_speed = 15;
		}
		if(this.Stopped_Counter >= this.STOPPED_INCREMENTER_LIMIT)
		{
			this.propulsion_speed = (this.propulsion_speed >= 30) ? 30 : this.propulsion_speed+1;
			this.Stopped_Counter = 0;
		}

		this.Previous_Distance = this.Distance_differential;

		if(this.prev_drive_action !== this.drive_action)
		{
			this.propulsion_speed = 15;
		}

		this.prev_drive_action = this.drive_action;

		this.execDrive(this.drive_action);
	}, this.DriveRate);

	/**Function Testing Section **/
	//var parent = this;
	//setTimeout(function(){parent.Distance_differential = 1},60000);
	// =====================================
	...
	react(input)
	{
		switch (input.mode)
		{
			...
			case "GATE": //write Gate GPS
				this.GPS_gate.lattitude = input.lattitude;
				this.GPS_gate.longitude = input.longitude;
				break;
	...
	{
		var parent = this;
		this.vision_process = this.exec(`./modules/NeoCortex/Vision/mainV2`, (error, stdout, stderr) =>
		{
			if(error)
			{
				this.feedback(error);
			...
			var distance_GPS = this.distanceGPS(
				this.GPS_current.lattitude,
				this.GPS_current.longitude,
				this.GPS_gate.lattitude,
				this.GPS_gate.longitude
			...
			this.Finish = "No";
			this.Distance_differential = distance_GPS;
			this.log.debug1("distance vision: " + distance_vision);
			if (distance_vision >= 1 || isNaN(distance_vision) === true)
			{
				if (distance_GPS >= 1.5 && distance_GPS != -1)
				{
					...
					{
						this.execDrive(direction_vision);
					}
					else
					{
						this.readGPS();
	...
	execDrive(direction)
	{
		switch (direction)
		{
			case 'L': //go left
				...
				this.AI_direction = 'Left';
				this.upcall("CALL", "DriveSystem",
				{
					speed: this.propulsion_speed,
					angle: -90,
					mode: "Y"
				});
				//setTimeout(function(){ this.upcall('STOP_AI')},500);
				break;
			case 'R': // go right
				this.log.output("Go Right");
				this.AI_direction = 'Right';
				this.upcall("CALL", "DriveSystem",
				{
					speed: this.propulsion_speed,
					angle: 90,
				...
				break;
			case 'C': //go forward
				this.log.output("Go Forwad");

				this.AI_direction = 'Forward';
				this.upcall("CALL", "DriveSystem",
				{
					speed: this.propulsion_speed,
					angle: 0,
	...
	pathGPS()
	{
		this.GPS_Heading = this.headingGPS(
			this.GPS_current.lattitude,
			this.GPS_current.longitude,
			this.GPS_gate.lattitude,
			this.GPS_gate.longitude
		...
		try
		{
			this.Tracker_Heading = Tracker_Object.heading;
		}
		catch (err)
		{
			this.log.output(err);
		...
		this.log.debug1("Heading differential: " + heading_differential);
