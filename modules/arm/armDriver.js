...
Arm.prototype.handle = function(input) { //Info is an object, with members outlined when sending control signals via arm interface html
	...
	// 	var hexdeg = angle(input.base); //grab the initial hex val of the degree position
	...
	if(input.shoulderL != undefined){
		// var hexdeg = angle(input.shoulderL); //grab the initial hex val of the degree position
		var hexdeg = (input.shoulderL/360) * 4095;
		var high = hexdeg >> 8; //grab the highbyte
		var low = hexdeg & 0xFF; //format hexdeg to have only the lowbyte
	...
	// 	var hexdeg = angle(input.shoulderR); //grab the initial hex val of the degree position
	// 	var high = hexdeg >> 8; //grab the highbyte
	// 	var low = hexdeg & 0xFF //format hexdeg to have only the lowbyte
	// 	control(WRITE, RIGHTSHOULDER, POSITION, low, high);
	// }
	// if(input.elbow != undefined){
	// 	var hexdeg = angle(input.elbow); //grab the initial hex val of the degree position
	// 	var high = hexdeg >> 8; //grab the highbyte
	// 	var low = hexdeg & 0xFF //format hexdeg to have only the lowbyte
	// 	control(WRITE, ELBOW, POSITION, low, high);
	// }
	// if(input.wrist != undefined){
	// 	var hexdeg = angle(input.wrist); //grab the initial hex val of the degree position
	// 	var high = hexdeg >> 8; //grab the highbyte
	// 	var low = hexdeg & 0xFF //format hexdeg to have only the lowbyte
	// 	control(WRITE, WRIST, POSITION, low, high);
	// }
	// if(input.speed != undefined){
	// 	var obj = motorSpeed(input.speed); //grab the initial hex val of the degree position
	// 	var high = hexdeg >> 8; //grab the highbyte
	// 	var low = hexdeg & 0xFF //format hexdeg to have only the lowbyte
...
function control(instruction, motorID, register, lowbyte, highbyte){ //parameters==object with motor IDs and values, use member finding to determine what to do
	...
	if(highbyte != 0xFFFF){
		...
		serial.write(String.fromCharCode(0xFF)); //ÿ Signature Byte Char
		serial.write(String.fromCharCode(motorID)); //ID Byte Char
		serial.write(String.fromCharCode(length)); //Length Byte Char
		serial.write(String.fromCharCode(instruction)); //Instruction Byte Char
		serial.write(String.fromCharCode(register)); //Register Address Byte Char
		serial.write(String.fromCharCode(lowbyte)); //Value/Lowbyte (depends on function call)
		serial.write(String.fromCharCode(highbyte)); //Highbyte
		serial.write(String.fromCharCode(checksum)); //Checksum
		serial.write(String.fromCharCode("-")); //For Arduino
	}
	else{
		serial.write(String.fromCharCode(0xFF)); //ÿ Signature Byte Char
		serial.write(String.fromCharCode(0xFF)); //ÿ Signature Byte Char
...
function angle(degree){
	var pos = parseInt(degree);
	var goal = (pos/360) * 4095;
	if(goal > 4095){
		goal = 4095;
	}
	else if(goal < 0){
